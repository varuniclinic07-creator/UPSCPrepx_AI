...
const DEFAULT_JOB_OPTIONS: Record<JobType, JobOptions> = {
  ...
  [JobType.SEND_PAYMENT_CONFIRMATION]: {
    attempts: 3,
    backoff: { type: 'exponential', delay: 1000 },
    priority: 10,
  },
  ...
  [JobType.GENERATE_LECTURE]: {
    attempts: 2,
    backoff: { type: 'exponential', delay: 15000 },
    priority: 2,
  },
  [JobType.COMPILE_LECTURE]: {
    attempts: 2,
    backoff: { type: 'exponential', delay: 10000 },
    priority: 2,
  },
...
export class WorkerQueueService {
  ...
  async addJob<T extends JobPayload>(
    ...
    const job = await this.queue.add(jobType, payload as T, {
      jobId,
      ...defaultOptions,
      ...options,
    });
